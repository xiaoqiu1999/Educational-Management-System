            ...
            >
                <li v-for="(groupItem, index) in group.submenu" :key="index">
                    ...
                    >
                        ...
                        <vs-chip class="ml-auto" :color="groupItem.tagColor" v-if="groupItem.tag">{{
                            groupItem.tag
                        }}</vs-chip>
                    </h-nav-menu-item>
                </li>
...
<script>
// import VNavMenuItem from './VerticalNavMenuItem'
                ...
                const func = item => {
                    if (item.submenu) {
                        item.submenu.forEach(subItem => {
                            if ((path === subItem.url || routeParent === subItem.slug) && subItem.url) {
                                open = true;
        ...
        hovered(val) {
            this.$nextTick(() => {
                if (val) {
                    ...
                    if (
                        window.innerHeight - dd.getBoundingClientRect().top - dd.getBoundingClientRect().height - 28 <
                        1
                    ) {
                    ...
                    if (
                        dd.getBoundingClientRect().left + dd.offsetWidth - (window.innerWidth - 16) >= 0 ||
