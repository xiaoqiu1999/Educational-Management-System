...
import VNavMenuGroup from './VerticalNavMenuGroup';
...
export default {
                ...
                const func = item => {
                    if (item.submenu) {
                        item.submenu.forEach(subItem => {
                            if (subItem.url && (path === subItem.url || routeParent === subItem.slug)) {
                                open = true;
    ...
    watch: {
        ...
        reduce(val) {
            const windowWidth = this.$store.state.windowWidth < 1200 ? 'no-nav-menu' : 'default';
            const verticalNavMenuWidth = val ? 'reduced' : windowWidth;
